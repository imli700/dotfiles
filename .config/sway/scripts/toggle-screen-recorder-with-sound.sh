...
RAW_PATH_FILE="/tmp/current_wf_recorder_sound.path"
...
if [ -f "$PID_FILE" ]; then
  ...
  if mv "$PID_FILE" "$CLAIMED_PID_FILE" 2>/dev/null; then
    ...
    echo "Stopping wf-recorder w/ sound (PID: $RAW_PID)..." >>"$WF_LOG_FILE"
    ...
    notify-send -t 3000 -i media-record "Stopped Recording" "Audio raw file saved. Transcoding..."
    ...
    (
      ...
      if [ $? -eq 0 ]; then
        rm -f "$RAW_FILE"
        notify-send -t 4000 -i video-x-generic "Converted Successfully" "With‑sound → WhatsApp‑ready:\n$WA_OUT"
      else
  ...
  # --- START ---
  echo "--- NEW SOUND RECORDING SESSION ---" >>"$WF_LOG_FILE"
  RAW_FILE="$REC_DIR/$(date +'%Y_%m_%d-%H%M%S').mkv"
  echo "Starting wf-recorder w/ sound → $RAW_FILE" >>"$WF_LOG_FILE"
  ...
  notify-send -t 3000 -i media-record "Screen Recording" "Started with sound. Raw file:\n$RAW_FILE"
fi
